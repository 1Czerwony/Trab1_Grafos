...
def BFS(g, s):                                  # Recebe Grafo g e vértice inicial s
    ...
    while q != []:                              # Enquanto fila q não for vazia
        u = q.pop(0)                           # Retira vértice da fila e guarda ele em u
...
def isCyclic(g):
    ...
    for u in g.adj:
        arestas += len(u)
    arestas = arestas/2
    print('número de vértices = ', g.vertices)
    print('numero de arestas = ', arestas)
    if arestas > (g.vertices-1):
        return True
    else:
        return False
